...
class Gaussian_connector(multiprocessing.Process):
    ...
    def run(self):
        ...
        if '.inp' in basename or '.gjf' in basename:
            ...
            while not run.longRunner().run(8,['pushd %s ; xrg09y %s ; popd'%(os.path.dirname(self.filename),'.'.join(basename.split('.')[:-1]))],[19,20,21,22,23,24,28]):
                pass
        else:
            #os.system('pushd %s ; xrg09y %s ; popd'%(os.path.dirname(self.filename),basename))
            while not run.longRunner().run(8,['pushd %s ; xrg09y %s ; popd'%(os.path.dirname(self.filename),basename)],[19,20,21,22,23,24,28,29]):
                pass
