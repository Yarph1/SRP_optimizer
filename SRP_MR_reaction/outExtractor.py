...
class extractor(object):
    ...
    def energy(self):
        ...
        for filer in self.prodOuts:
            ...
            print (__file__.split('/')[-1]+':\t'+' prodOuts:: '+str(filer)+'\t'+str(self.extracter('scfenergies',filer)[-1]*self.prodStoich[count][0]))
            count+=1
        count=0        

        for filer in self.reacOuts:
            en-=self.extracter('scfenergies',filer)[-1]*self.reacStoich[count][0]
            print (__file__.split('/')[-1]+':\t'+' reacOuts:: '+str(filer)+'\t'+str(self.extracter('scfenergies',filer)[-1]*self.reacStoich[count][0]))
            count+=1
        print (__file__.split('/')[-1]+':\t'+' Total Energy difference:: '+str(filer)+'\t'+str(en))
        return en
